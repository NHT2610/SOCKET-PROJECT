...
void ConnectThread::operator()(CSocket& client, int NumberCode)
{
	...
	while (true) {
		...
		if (strcmp(temp1, "LOGIN") == 0) {
			...
			vector<string> account = split_string(temp2, '|');
			
		}
		//Yêu cầu đăng ký tài khoản từ client
		else if (strcmp(temp1, "RIGISTER") == 0) {

		}
		//Yêu cầu thoát từ client
		else if (strcmp(temp1, "QUIT") == 0) {
			cout << "Client " << NumberCode << " da dong ket noi!\n";
			break;
		}
	}
	client.Close();
